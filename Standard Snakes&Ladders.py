...
number_of_times = 1000000000


def cl_markov_matrix(max_roll=6, max=100):
...
plt.hist(sim_games, bins=range(200))
plt.xlabel('Number of Turns to win')
plt.title('Simulated Lengths of Chutes & Ladders Games '+str(number_of_times)+ ' times')
print("Min Count: " + str(sim_games.count(7)))
print("Max Count: " + str(max(sim_games)))
print("Count of main ladder taken = " + str(count_ladder_main) + " % of games = "+ str(count_ladder_main/number_of_times))
plt.show()

plt.hist(sim_games, bins=range(200), cumulative=True)
plt.grid(True)
plt.xlabel('Number of Turns to win')
...
print(np.mean(sim_games))
print(np.median(sim_games))
plt.hist(sim_games, bins=range(30))
plt.xlabel('Number of Turns to win')
